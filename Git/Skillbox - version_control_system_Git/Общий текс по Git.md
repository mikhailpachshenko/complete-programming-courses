...
`git blame` 'fileName.ext'  **— просмотреть изменения в конкретном файле  (кто, когда, коммит)** 
...
`git clone` - **копируем (клонируем) репозиторий с удалённого сервера**`

`git clean -f` **— отменяем файлы которые еще не были добавлены в индекс но есть в директории**

***git commit –m 'comment'*** - сохраняем внесенные в индекс изменения с сопровождающим текстом об изменениях ()

`git diff` '*file.ext*' **— просмотреть изменения в конкретном файле** 
...
`git rm -cached` fileName.ext **— удаляем добавленный в индекс и закоммиченный файл**
`-cached` **— флаг, не удаляем из репозитория, а перестаем его отслеживать** 

**`git restore` *'nameFile.ext'* **— отменяем изменения не внесённые в индекс и коммит**
`git restore --staged` *'nameFile.ext'* **— убираем изменения из индекса**
...
`git restore` <file.ext> **— удаляем изменения не добавленные в индекс **
`git restore -stage` <file.ext> **— удаляем изменения из индекса но не из директории**
